...
def _build_gt_lookups(
    ...
    for src in (gt.get("lines", []), gt.get("segments", [])):
        for entry in src:
            ...
            li_int = int(li)
            by_line.add(li_int)
            sid = entry.get("shabad_id", gt_shabad_id)
            if sid is not None:
...
def score_video(gt: dict, pred: dict, collar: int = 1, score_gaps: bool = True) -> dict:
    ...
    #
    # Frame classification for scoring:
    #
    #   GT:  ... line_A ...][ collar ][ line_B interior ][ collar ][... line_C ...
    #                        ↑ boundary                   ↑ boundary
    #
    #   - Interior frames: must match GT label exactly
    #   - Collar frames (within `collar` seconds of a GT boundary):
    #     accept {line_before, line_after, None} — tolerates tracker lag
    #   - Gap frames (between GT segments):
    #     accept {adjacent_before, adjacent_after, None}
    #   - Unlabeled frames (outside all segments): accept anything
    #
    boundary_acceptable = {}
...
def build_card(gt: dict, pred: dict, collar: int = 1,
    ...
    The returned dict matches the CARDS schema used by the
    visualizer web page and by ``visualize.py``.
